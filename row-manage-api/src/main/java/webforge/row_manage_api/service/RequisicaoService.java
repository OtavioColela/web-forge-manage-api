...
public class RequisicaoService {
    ...
    public void definirRequisicao(Long id, StatusPedido statusPedido) {
        ...
        if (statusPedido.equals(StatusPedido.APROVADO)) {
            ...
            for (ItemPedido item : requisicao.getItemPedido()) {
                ...
                // BUG 9: baixa estoque antes de salvar status
                estoqueService.reduzirMaterial(
                        item.getMaterialEntity().getId(),
                        item.getQuantidade()
                );
            }
        }
    ...
    public List<RequisicaoResponse> getAllRequisitionsByUser(Long id) {
        ...
        var requisicoes =
                requisicaoRepository.findBySolicitanteIdOrderByDataRequisicaoDesc(id);
    ...
    public List<RequisicaoResponse> getAllRequisitionsByStatus(StatusPedido statusPedido) {
        return RequisicaoMapper.toResponse(
                requisicaoRepository.findByStatusPedidoOrderByDataRequisicaoDesc(statusPedido)
        );
    }
